...
import info
import pandas as pd
# the "as" is how you call it
# pandas is an essential dataset database hehe
...
def linksSection():
    ...
    #call first parameter(string with html code), second parameter(lets them know you're using html)
    st.sidebar.text("Checkout my work")
    github_link = f'<a href="{info.my_github_url}"><img src="{info.github_image_url}" alt="Github" width="65" height="75"></a>'
...
def educationSection(education_data, course_data):
    ...
    st.dataframe(coursework, column_config={
        ...
        hide_index=True, #if this is false, streamlit adds a column that's an index, like 0, 1, 2...
        )
    st.write("-----")
    # ** is bold, make sure u have a space at the end like "**Bold Text** and other shit"
    # dataframe makes a table
...
def experienceSection(experience_data):
    ...
    for job_title, (job_description, image) in experience_data.items():
        expander = st.expander(f"{job_title}") #built in function
        expander.image(image, width=250)
        for bullet in job_description:
            expander.write(bullet) #makes it look like a bullet
    st.write("----")
...
def skillsSection(programming_data, spoken_data):
    ...
    for skill, percentage in programming_data.items():
        ...
        st.progress(percentage) #progress bar
...
def activitiesSection(leadership_data, activity_data):
    ...
    with tab1:
        ...
        for title, (details, image) in leadership_data.items():
            ...
            expander.image(image, width=250) #because we're writing in expander, don't do st.thin, do expander.thing
...
# info.thing while calling so it knows we have to access info data
